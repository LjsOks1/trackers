...
filetype=Class(db,"filetype",
                name=String(),
                shortname=String())
filetype.setkey("shortname")
...
tasktype = Class(db,"tasktype",
            name=String(),
            shortname=String())
tasktype.setkey("shortname")
...
# Classes for TPM usecase

partnertype=Class(db,"partnertype",
                name=String())
partnertype.setkey("name")

language=Class(db,"language",
                name=String(),
                shortname=String())
language.setkey("shortname")

partner = Class(db, "partner",
                name=String(),
                partnertype=Link("partnertype"))
partner.setkey("name")

series =  Class(db, "series",
                title=String(),
                client=Link("partner"))

episode = IssueClass(db,"episode",
                keyword=Multilink("keyword"),
                priority=Link("priority"),
                status=Link("status"),
                duration=Integer(),
                series=Link("series"),
                order=Integer(), #Episode number!!!!
                barcode=String(),
                screenerid=String(),
                deadline=Date(),
                client=Link("partner"))

order= IssueClass(db,"order",
            orderid=String(),
            partner=Link("partner"),
            orderdate=Date(),
            status=Link("status"))

projecttype=Class(db,"projecttype",
                  name=String(),
                  shortname=String())
...
#SHA: ef401f694f1ec55658359d1fee342675b4d199cb
